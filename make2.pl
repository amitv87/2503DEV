...
&cktheMF($theMF);
if(defined $project_makefile_ext && $project_makefile_ext ne "" && $project_makefile_ext ne "NONE") {
...
my $targetFolder = "build\\${custom}";
if(defined $project_makefile_ext && $project_makefile_ext ne "" && $project_makefile_ext ne "NONE") {
	$concatenate_proj_mak = "${targetFolder}\\_${custom}_${project}.mak";
	...
	my $return_code = system("${makeCmd} -f${makeFolder}${myMF} -r -R CUSTOMER=$custom PROJECT=$project $gen_concate_option gen_concatenate_proj");
...
#add by MagicNing for PROJECT
print "\n ------------------------------------------------------------------  \n";
print "***************************   add for project  ***************************";

# PROJECT_SUPPORT_TRACK
if (defined($project_support_track)) 
{
	#这里复制mak文件晚了点，直接在编译前复制  chengj
	#system("copy /Y make\\${custom}_${project}_${project_support_track}.mak make\\${custom}_${project}.mak > nul");
	#print "\n copy /Y make\\${custom}_${project}_${project_support_track}.mak make\\${custom}_${project}.mak > nul ";

	system("copy  /Y custom\\codegen\\${custom}_BB\\codegen_${project_support_track}.dws custom\\codegen\\${custom}_BB\\codegen.dws > nul");		
	print"\n copy  /Y custom\\codegen\\${custom}_BB\\codegen_${project_support_track}.dws custom\\codegen\\${custom}_BB\\codegen.dws > nul";		

	print "\n******************************      End     ******************************  \n";
	print " ------------------------------------------------------------------  \n";
}
#foreach $para ( @arguments )
#{
#if ( ($para eq "track") || ($para eq "track_nvram") ) {
  print ("\n\n 删除track/track_agps/track_nvram的obj目录，强制代码重新编译！ \n\n");
  system("RMDIR /S /Q build\\${custom}\\${project}\\MT6261o\\track");
  #system("RMDIR /S /Q build\\${custom}\\${project}\\MT6261o\\track_agps");
  #system("RMDIR /S /Q build\\${custom}\\${project}\\MT6261o\\track_nvram");
#}
#}
...
foreach my $action (@theAct) {
  ...
  if ($action eq "ts_bm_new"){
	system("echo BM_NEW=TRUE >> ${makeFolder}~buildinfo.tmp");
	$result = system("${makeCmd} -f${makeFolder}${myMF} -k -r -R CUSTOMER=$custom PROJECT=$project $target_option ts_new");
  }
  ...
  if ( $action eq "fota_tracker") {
    
    if ( $scat_txt eq "new"  ||  $scat_txt eq "NEW")
    {
      $result = system("perl tools\\AutoGenFotaScat.pl ");
      if($result != 0)
      {
	die "Error: Scatter auto make failed  --  error: $result!!!\n";
	exit 1;
      }
      else
      {
    ...
    elsif( $scat_txt eq "remake" || $scat_txt eq "REMAKE")
    {
      $result = system("perl tools\\FOTA_Tracker.pl build\\${custom} ${project}  ");
    
      if($result != 0)
      {
	die "Error: fota tracker make failed  --  error: $result!!!\n";
	exit 1;
      }
      else
      {
    ...
    elsif( $scat_txt eq "newall" || $scat_txt eq "NEWALL" || $scat_txt eq "A" || $scat_txt eq "a")
    {
        $result = system("perl tools\\AutoGenFotaScat.pl ");
        if($result != 0)
        {
    ...
    {
        die "Error: fota tracker make failed  --  error: 指令参数错误，本指令支持一下参数 [NEW] [new]  [REMAKE]  [remake] !!!\n";
	exit 1;
    }
  }
...
sub error_handle
{
  ...
  my $flag = 0;
  foreach (@files) {
  	next if($_ =~ /BOOTLOADER|FOTA|TINY/i);
